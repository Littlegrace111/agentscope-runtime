...
@app.route("/browser/resource_url", methods=["GET"]) 
async def get_browser_resource_url():
    session_id = session.get("id", None)
    if session_id and session_id in agents and not agents[session_id].is_closed:
        resource_url = getattr(agents[session_id], "resource_url", "") or ""
        return jsonify({"resource_url": resource_url}), 200
    return jsonify({"resource_url": ""}), 200
