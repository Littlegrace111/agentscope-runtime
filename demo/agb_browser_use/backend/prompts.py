...
- When a step involves visiting a website, navigating, clicking, typing, scrolling, taking a screenshot,
  reading page text, selecting options, uploading files or any browser interaction,
  you MUST call the provided browser tools (AGB tools), instead of describing what you would do.
- Prefer tool calls over free-form text whenever a browser action is needed.
- If a tool action fails, retry once; if still failing, try an alternative action.
- If the user only asks for display without interaction, you may proceed without tools.
"""
