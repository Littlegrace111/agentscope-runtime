...
const BACKEND_BROWSER_RESOURCE_URL = REACT_APP_API_URL + "/browser/resource_url";
...
const App: React.FC = () => {
  ...
  // When CDP endpoint (webSocketUrl) is available, try fetch resource_url for iframe rendering
  useEffect(() => {
    if (webSocketUrl) {
      console.log("[frontend] webSocketUrl updated:", webSocketUrl);
      get_resource_url();
    }
  }, [webSocketUrl]);
  ...
  async function get_ws() {
    ...
    console.log("[frontend] fetched webSocketUrl:", data.url);
    setWebSocketUrl(data.url);
  }

  async function get_resource_url() {
    const response = await fetch(BACKEND_BROWSER_RESOURCE_URL, {
      method: "GET",
    ...
    const data = await response.json();
    console.log("[frontend] fetched resource_url:", data.resource_url);
            ...
            <Flex gap={"large"}>
              ...
              {resourceUrl ? (
                ...
                <div style={{width:"100%", height:"80vh", display:"flex", alignItems:"center", justifyContent:"center"}}>
                  Waiting for AGB browser resource...
                </div>
              )}
            </Flex>
          </Flex>
